...
export async function artifact(browser) {
  const keymap = (await readFile(resolve(root, 'src/keymap.js'), 'utf8')).replace(/^export /gm, '');
  const content = await readFile(resolve(root, 'src/content.js'), 'utf8');
...
export async function build(output = resolve(root, 'dist')) {
...
export async function verifyBuild(browser, output = resolve(root, 'dist')) {
...
export async function sourceRevision() {
  ...
  for (const path of untracked) digest.update(JSON.stringify(path)).update(await readFile(resolve(root, path)));
